...
async function fetchCustomRange() {
  try {
    ...
    const response = await fetch(
      `/api/summary?start_timestamp=${startTimestamp}&end_timestamp=${endTimestamp}`,
    );
    const data = await response.json();
    createCpuUsageChart(data, "custom");
    return data;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("startTime").setAttribute("max", maxDateTime);
  document.getElementById("endTime").setAttribute("max", maxDateTime);
});
...
async function loadSummaryData(mode) {
  try {
    ...
    const response = await fetch(
      `/api/summary?start_timestamp=${timeRange.start}&end_timestamp=${timeRange.end}&interval_type=${interval_type}`,
    );
    const data = await response.json();
